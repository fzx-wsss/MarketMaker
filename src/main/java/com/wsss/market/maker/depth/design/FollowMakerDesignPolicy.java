...
@Component
@Scope("prototype")
public class FollowMakerDesignPolicy implements MakerDesignPolicy {
    @Setter
    ...
    private void followPlaceOrCancelOrder(MakerContext bbo) {
        ...
        Map<BigDecimal, List<Depth>> followBuyDepths = subscribedOrderBook.getBuyBooks(bbo.getMakeBestBuy(), maxLevel)
                .stream().collect(Collectors.groupingBy(
                        d -> d.getPrice().setScale(priceScale, BigDecimal.ROUND_DOWN)
                ));
        Map<BigDecimal, List<Depth>> followSellDepths = subscribedOrderBook.getSellBooks(bbo.getMakeBestSell(), maxLevel)
                .stream().collect(Collectors.groupingBy(
                        d -> d.getPrice().setScale(priceScale, BigDecimal.ROUND_DOWN)
                ));
    ...
    private void addPlaceOrders(Map<BigDecimal, List<Depth>> followDepths, Set<BigDecimal> ownerPrices, MakerContext makerContext, Side side) {
    ...
    private void addRemoveOrders(Set<BigDecimal> ownerPrices, Map<BigDecimal, List<Depth>> followDepths, MakerContext makerContext, OwnerOrderBook ownerOrderBook, Side side) {
    ...
    private void avoidOrEatUserOrder(MakerContext makerContext) {
    ...
    private MakerContext getMakerContext() {
